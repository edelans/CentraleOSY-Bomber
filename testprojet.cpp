...
void drawing::initialize(const string file) const
{
	...
	int d�calage_X = (LONGUEUR_MAX - longueur_niveau - 3)*TAILLE_CASE/2;
	int d�calage_Y = (LARGEUR_MAX - longueur_niveau - 3)*TAILLE_CASE/2;
	...
	while (is >> nombre) // parcours de chaque case cod�e dans le fichier de niveau
	{
		...
		{
		case 1:
			game.draw_image((NumeroColonne - 1/2)*TAILLE_CASE + d�calage_X,(NumeroLigne - 1/2)*TAILLE_CASE + d�calage_Y,sol);
			break;
		case 2:
			game.draw_image((NumeroColonne - 1/2)*TAILLE_CASE + d�calage_X,(NumeroLigne - 1/2)*TAILLE_CASE + d�calage_Y,murdestr);
			break;
		case 3:
			game.draw_image((NumeroColonne - 1/2)*TAILLE_CASE + d�calage_X,(NumeroLigne - 1/2)*TAILLE_CASE + d�calage_Y,mur);
			break;
		default:
